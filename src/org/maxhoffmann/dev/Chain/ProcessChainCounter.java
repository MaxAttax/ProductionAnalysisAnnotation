...
public class ProcessChainCounter {
	...
	public void ProcessChainOperations(ArrayList<String> generatedChains) {
		...
		// Counter for process chains of a certain length, which is determined in the optimization algorithm
		ArrayList<Integer> countProcessChainsLength = new ArrayList<Integer>(); 

		// Counter for process chains, that contain one of the distinct determined process chains
		ArrayList<Integer> countContainProcessChains = new ArrayList<Integer>(); 

		// Counter for Sub Process Chains within the distinctive determined process chains
		...
		// within all listed chains without counting the distinctive chains itself
		ArrayList<Integer> countContainChainsDiff = new ArrayList<Integer>(); 

		// Counter for Sub Process Chains within the different process chains without counting the chains itself
		ArrayList<Integer> countSubChainsDiff = new ArrayList<Integer>(); 

		
		// The chain counter types like above, just counting the chains of a certain length
		...
		/**
		 * This algorithm counts the total number of all process chains for each
		 * length. For example: number of 1-link chains, number of 2-link
		 * chains, etc. within the whole process chain list.
		 */
		...
		/**
		 * Calculation of the percentage amount of each chain-length category
		 * compared to all chains.
		 */
		...
		for (int c = 0; c < maxProcessChainLength; c++) {
			...
			LOGGER.info((c + 1) + "er-Kette:\t"
					+ countLengthProcessChains.get(c) + "\t Anteil an Gesamtzahl:  "
					+ percentChainCount.get(c) * 100 + " %");
		}
		LOGGER.info("\nTotal number of chains: " + listedChains.size() + "\n");

		// Initialization of cumulated percentage value of all chains smaller or equal a certain chain length.
		double cumulatedLengthPercentage = 0; 
		
		// Counter variable in order to determine the longest chain with a sufficient amount within all process chains.
		int chainCounter = 1; 
		...
		/**
		 * The longest chain type of the cumulated chains is taken in order to
		 * generate the main process chains. Condition: The longest chain type
		 * make up more than 10 % of all chains, because it has to be
		 * representative. If for example, the 5-link chain is the last chain of
		 * the cumulated chains until 90 %, but makes up less than 10 %, than
		 * the 4-link chain is taken in order to generate the main process
		 * chains, and so on...
		 */

		int actualPercentageIndex = 0; 	// index of the List that contains the
										// percentage values of each chain
										// length
		float actualPercentage = 0; 	// Declaration of the actual percentage
										// value according to the index of the
										// percentage list.
		int arrayNumber = 1; 			// Variable in order to search for the percentage
										// value that fits to the given conditions.

		LOGGER.info("Start-Kette:\t"
				+ percentChainCount.get(chainCounter - 1 - arrayNumber));

		actualPercentage = percentChainCount
				.get(chainCounter - 1 - arrayNumber); 				// Initialization of the actual percentage value
		...
		/*
		 * Generation of a list that contains all process chain of the
		 * determined length (no duplicates).
		 */
		...
		/**
		 * count the number of process chains, that match the process chains of
		 * a certain length within all listed chains.
		 */
		...
		/**
		 * countContainPrcoessChainsLength - how many times can distinctive
		 * chains of a certain length be found within all process chains
		 * countSubProcessChainsLength - how many of the listed chains can be
		 * found within each distinctive process chain
		 * countContainChainsDiffLength - counterSub excluding the chains itself
		...
		SortedSet<ProcessChainObject> set = new TreeSet<>();
		
		for (int j = 0; j < processChainsLength.size(); j++) {
			set.add(new ProcessChainObject(
				processChainsLength.get(j),
				countProcessChainsLength.get(j),
				countSubChainsDiffLength.get(j)
			));
		...
		/**
		 * for ( int m = 0; m < listedChains.size(); m++ ) { int
		 * counterContainBreak = 0; for ( n = 0; n < processChains.size(); n++ )
		 * { if ( listedChains.get(m).contains(processChains.get(n)) == true ) {
		 * counterContainBreak++; }
